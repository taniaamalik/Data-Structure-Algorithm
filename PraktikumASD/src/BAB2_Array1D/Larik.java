...
// Lengkapi bagian ini
        double[] s = new double[l1.size + l2.size];
        Larik sambung = new Larik(s);
        int counter = 0;
        for (int i = 0; i < l1.size; i++) {
            sambung.isiItem(counter, l1.getItem(i));
            counter++;
        }
        for (int i = 0; i < l2.size; i++) {
            sambung.isiItem(counter, l2.getItem(i));
            counter++;
        }
        return sambung;
    }
...
// lenkapi bagian ini
        ...
        Larik copy = new Larik(c);
        for (int i = k; i < k + n; i++) {
            copy.isiItem(i, l.getItem(i));
        }
        return copy;
    }
...
// lenkapi bagian ini
    ...
    public static double LarikKaliLarik(Larik x, Larik y) {
        double hasil = 0;
        if (x.getSize() == y.getSize()) {
            for (int i = 0; i < x.getSize(); i++) {
                hasil += x.getItem(i) * y.getItem(i);
            }
        }
        return hasil;
    }
